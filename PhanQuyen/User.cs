    ...
    public partial class User : Form
    {
        ...
        public List<tabprivs> loadlistptb()
        {
            ...
            using (OracleConnection connection = new OracleConnection(ConfigurationManager.ConnectionStrings["con"].ConnectionString))
            {
                ...
                string query1 = "SELECT * FROM DBA_ROLE_PRIVS where GRANTEE = '" + name + "'";
                connection.Open();
                ...
                {
                    DataTable data = new DataTable();
                    string query = "SELECT * FROM  ROLE_TAB_PRIVS  where Role = '" + data1.Rows[j]["GRANTED_ROLE"].ToString() + "'";
                    OracleCommand cmd = new OracleCommand(query, connection);
                    data.Load(cmd.ExecuteReader());
                    for (int i = 0; i < data.Rows.Count; i++)
                    {
            ...
            using (OracleConnection connection = new OracleConnection(ConfigurationManager.ConnectionStrings["con"].ConnectionString))
            {
                ...
                for (int i = 0; i < data.Rows.Count; i++)
                {
                    ...
                    if (data.Rows[i]["TYPE"].ToString() == "TABLE" && ((data.Rows[i]["PRIVILEGE"].ToString() == "SELECT") || (data.Rows[i]["PRIVILEGE"].ToString() == "UPDATE")))
                    {
                        ...
                        DataTable dataview = new DataTable();
                        string queryview = "SELECT distinct COLUMN_NAME FROM USER_TAB_COLUMNS WHERE table_name ='" + data.Rows[i]["TABLE_NAME"].ToString() + "'";
                        OracleCommand cmdview = new OracleCommand(queryview, connection);
                        dataview.Load(cmdview.ExecuteReader());
        ...
        private void dongy_Click(object sender, EventArgs e)
        {
            ...
            {
                connection.Open();
                string query2 = "begin Grant_NewUser('" + name + "','123'); end;";
                OracleCommand command2 = new OracleCommand(query2, connection);
                int result = command2.ExecuteNonQuery();
                connection.Close();
            }
            if (insert.Checked == true)
                ...
                {
                    connection.Open();
                    string query2 = "grant insert on " + table.Text + " to " + name;
                    OracleCommand command2 = new OracleCommand(query2, connection);
                    int result = command2.ExecuteNonQuery();
                    connection.Close();
                }
            if (delete.Checked == true)
                ...
                {
                    connection.Open();
                    string query2 = "grant delete on " + table.Text + " to " + name;
                    OracleCommand command2 = new OracleCommand(query2, connection);
                    int result = command2.ExecuteNonQuery();
                    connection.Close();
                }
            using (OracleConnection connection = new OracleConnection(ConfigurationManager.ConnectionStrings["con"].ConnectionString))
            {
                ...
                if (select != "")
                {
                    string query2 = "Create or replace view VIEW_" + table.Text + "_" + name + " as SELECT " + select + " from " + table.Text;
                    OracleCommand command2 = new OracleCommand(query2, connection);
                    int result = command2.ExecuteNonQuery();
                ...
                if(update!="")
                {
                    string query2 = "grant update("+update+") on VIEW_" + table.Text + "_" + name + " to " + name;
                    OracleCommand command2 = new OracleCommand(query2, connection);
                    int result = command2.ExecuteNonQuery();
                }
